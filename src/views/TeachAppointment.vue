<template>
  <div>
    <el-card shadow="hover" style="min-height: 600px;">
      <template #header>
        <div class="clearfix">
          <span>教师预约信息表(有效预约)</span>
        </div>
      </template>
      <div class="info">
        <el-table
          :data="teachAppointData"
          style="width: 100%"
          height="420"
          border
        >
          <el-table-column property="teach_id" label="编号" />
          ...
          <el-table-column property="appoint_type" label="预约类型" />
    ...
    <!-- 拒绝预约申请时的弹出框 -->
    <el-dialog v-model="dialogFormVisible" :title="`拒绝${form.teach_name}的预约申请`" center>
      <el-form :model="form">
...
// 查询所有教师预约信息
const findTeachAppoint = async () => {
  sysVar.value = await axios.get('/sys-var/findAll');
  const open_date = sysVar.value.open_date;
  const data = await axios.get('/appointment/findTeachAppoint');// 获取所有预约信息
  teachAppointData.value = appointFilter(data, open_date).validAppointArr;// 未过期的预约
}
...
const generateMsg = (lab_name, lab_location, week, day, parid, start_time, reason) => {
  ...
  return {
    successMsgContent,// 预约成功消息
    failMsgContent,// 预约被拒消息
  };
}

// 同意单个预约
const agreeAppointment = ({id, user_id, lab_name, lab_location, week, day, parid, start_time}) => {
  axios.post('/appointment/agreeTeachAppoint', {
    id
  })
  .then(res => {
    ElMessage.success(res);
    findTeachAppoint();// 更新页面数据
    // 添加预约成功消息到用户消息记录。
...
const agreeAllAppoint = () => {
  // 获取当前表格数据数组中未处理的所有预约
  const unhandleAppoints = teachAppointData.value.filter(item => item.appoint_stat === '未处理');
  ...
  if(idArr.length === 0) {
    ...
    .then(() => {
      // 确定
      axios.post('/appointment/agreeAllTeachAppoint', {
        idArr
      })
      .then(res => {
        ElMessage.success(res);
...
// 拒绝预约
const refuseAppointment = ({id, teach_name}) => {
  form.value.id = id;
  form.value.teach_name = teach_name;
...
const submitRefuseReason = () => {
  if(!form.value.reason) {
    ...
    axios.post('/appointment/refuseTeachAppoint', {
      id: form.value.id,
    })
    .then(res => {
      findTeachAppoint();// 更新页面数据
      ...
      const user_type = 'teacher';
      const pub_datetime = new Date().toLocaleString();
      const {lab_name, lab_location, week, day, parid, start_time, user_id} = teachAppointData.value.find(item => item.id === form.value.id);
